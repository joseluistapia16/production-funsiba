...
export async function createUser(req: Request, res: Response) {
  ...
  try {
    ...
    await conn.query<RowDataPacket[]>(
      `CALL bgzqyzz5g5aktydlzuez.sp_create_user('${user.username}', ${user.avatar_id}, @error);`
    );
...
export async function getAllUsers(req: Request, res: Response) {
  try {
    ...
    const result = await conn.query<RowDataPacket[]>(
      `SELECT user.*, avatar.url FROM bgzqyzz5g5aktydlzuez.user, bgzqyzz5g5aktydlzuez.avatar WHERE user.avatar_id = avatar.avatar_id;`
    );
...
export async function getUser(req: Request, res: Response) {
  ...
  try {
    const conn = await connect();
    const result = await conn.query<RowDataPacket[]>(
      "SELECT u.*, a.url FROM bgzqyzz5g5aktydlzuez.user u , avatar a  WHERE u.avatar_id = a.avatar_id AND user_id = ?;",
      [user_id]
    );
    res.status(200).json(result[0][0]);
  } catch (e) {
